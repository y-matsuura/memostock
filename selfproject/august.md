# 情報収集
## firebase
https://techblog.zozo.com/entry/firebase-realtime-questionnaire

https://blog.ikedaosushi.com/entry/2019/04/17/201246

https://qiita.com/pannpers/items/244a7e3c18d8c8422e4f

## clound9
### AWS Cloud9でSeleniumからHeadless Chromeを使うまで【Python3】
https://steemit.com/japanese/@kakel/aws-cloud9-selenium-headless-chrome-python3

### Cloud9上でクローラーを作るための環境構築
https://qiita.com/grm/items/b87f1b27452b63cc29ee

###  スクレイピングをPythonでするときは、Cloud9・Selenium・Herokuで決まり！誰でもできるスクレイピングマニュアル 
https://review-of-my-life.blogspot.com/2017/09/selenium-5-minutes-setup-on-cloud9.html

### AWS Cloud9でCapybara+Selenium+Chrome設定時のエラーとその対処法について[ Everyday Rails 6章 ]
https://kuracux.hatenablog.jp/entry/2018/12/30/182617

### Google Cloud FunctionsでPython+seleniumでスクレイピングしてみる(Part.3)
https://blowup-bbs.com/gcp-cloud-functions-python3/

## seleniumu
### 公式
https://www.seleniumhq.org/projects/

### 公式github
https://github.com/SeleniumHQ/selenium/wiki/Grid2

### Awesome Selenium : 素晴しい Selenium ライブラリの数々
https://qiita.com/hatai/items/a4cbff763b9ee8a7879a

### テスト自動化環境をセットアップする
https://developer.mozilla.org/ja/docs/Learn/Tools_and_testing/Cross_browser_testing/Your_own_automation_environment

### iOS 13のSafariブラウザが「WebDriver」を正式サポート。SeleniumなどによるUIテストの自動化が容易に
https://www.publickey1.jp/blog/19/ios_13safariwebdriverseleniumui.html

### TypeScript + WebdriverIO + Seleniumでテストを自動化しよう
https://www.casleyconsulting.co.jp/blog/engineer/296/

### 40通り以上の自動マルチブラウザテストをSelenium x CircleCI x BrowserStackで実現する
https://tech.plaid.co.jp/multi-browesr-test/

### Docker, Python, Selenium, Headless Chromeを用いたスクレイピング
https://lifedevops.com/?p=173

### Docker × Android エミュレータで、自動テスト(Appium)を並列化・爆速にする環境を作ったお話
https://tech.mercari.com/entry/2018/12/10/060000

### SELENIUM GRID IN THE CLOUD
https://www.gridlastic.com/

### Web UIテスト自動化の実行環境をSelenium Gridで
https://techblog.zozo.com/entry/qa-webui-test-automation-01

## その他
### ブラウザテスト自動化クラウドサービス『ShouldBee』
http://c16e.com/shouldbee/

### docker-composeのバージョン確認・バージョンアップ手順
https://qiita.com/kawamura_kenji/items/b6d133a5c4ad203e0fe5
